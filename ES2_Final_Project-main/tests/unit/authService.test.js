...
describe("authService", () => {
  ...
  it("rejects registration when the email is invalid", async () => {
    await expect(
      authService.register({
        name: "Invalid Email",
        email: "invalid-email",
  ...
  it("rejects registration when the password is too short", async () => {
    await expect(
      authService.register({
        name: "Short Password",
        email: "short-password@greenherb.pt",
        password: "Short1",
        role: "MANAGER"
      })
    ).rejects.toMatchObject({
  ...
  it("rejects registration when no payload is provided", async () => {
  ...
  it("rejects login when the email is missing but the password is present", async () => {
    await expect(
      authService.login({
        password: "StrongPass1"
      })
  ...
  it("rejects login when no payload is provided", async () => {
  ...
  it("rejects refresh when no payload is provided", async () => {
    await expect(authService.refresh()).rejects.toMatchObject({
      statusCode: 400,
      message: "Refresh token is required."
    });
